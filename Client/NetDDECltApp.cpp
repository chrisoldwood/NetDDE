...
bool CNetDDECltApp::OnConnect(const tchar* pszService, const tchar* pszTopic)
{
	...
	if (pService != nullptr)
	{
		try
		{
			...
			NetDDEPacketPtr requestPacket = EncodeCreateConversationPacket(pService->m_oCfg.m_strRemName, pszTopic);
...
void CNetDDECltApp::OnDisconnect(CDDESvrConv* pConv)
{
	...
	if (pService != nullptr)
	{
		...
		{
			// Create conversation message.
			NetDDEPacketPtr packet = EncodeDestroyConversationPacket(pNetConv->m_hSvrConv,
                                                                     pNetConv->m_nSvrConvID);
...
bool CNetDDECltApp::OnRequest(CDDESvrConv* pConv, const tchar* pszItem, uint nFormat, CDDEData& oData)
{
	...
	if ( (pService != nullptr) && (pService->m_oConnection.IsOpen()) )
	{
		...
		{
			// Create conversation message.
			NetDDEPacketPtr requestPacket = EncodeRequestItemPacket(pNetConv->m_hSvrConv,
			                                                        pNetConv->m_nSvrConvID,
			                                                        pszItem,
			                                                        nFormat);
...
bool CNetDDECltApp::OnAdviseStart(CDDESvrConv* pConv, const tchar* pszItem, uint nFormat)
{
	...
	if ( (pService != nullptr) && (pService->m_oConnection.IsOpen()) )
	{
		...
		{
			// Create conversation message.
			NetDDEPacketPtr requestPacket = EncodeStartAdvisePacket(pNetConv->m_hSvrConv,
			                                                        pNetConv->m_nSvrConvID,
			                                                        pszItem,
			                                                        nFormat,
			                                                        pService->m_oCfg.m_bAsyncAdvises,
			                                                        pService->m_oCfg.m_bReqInitalVal);
...
void CNetDDECltApp::OnAdviseStop(CDDESvrConv* pConv, CDDELink* pLink)
{
	...
	if ( (pService != nullptr) && (pService->m_oConnection.IsOpen()) )
	{
		...
		{
			// Create conversation message.
			NetDDEPacketPtr packet = EncodeStopAdvisePacket(pNetConv->m_hSvrConv,
			                                                pNetConv->m_nSvrConvID,
			                                                pLink->Item(),
			                                                pLink->Format());
...
bool CNetDDECltApp::OnExecute(CDDESvrConv* pConv, const CString& strCmd)
{
	...
	if ( (pService != nullptr) && (pService->m_oConnection.IsOpen()) )
	{
		...
		{
			// Create conversation message.
			NetDDEPacketPtr requestPacket = EncodeExecuteCommandPacket(pNetConv->m_hSvrConv,
			                                                           pNetConv->m_nSvrConvID,
			                                                           strCmd);
...
bool CNetDDECltApp::OnPoke(CDDESvrConv* pConv, const tchar* pszItem, uint nFormat, const CDDEData& oData)
{
	...
	if ( (pService != nullptr) && (pService->m_oConnection.IsOpen()) )
	{
		...
		{
			// Create conversation message.
			NetDDEPacketPtr requestPacket = EncodePokeItemPacket(pNetConv->m_hSvrConv,
			                                                     pNetConv->m_nSvrConvID,
			                                                     pszItem,
			                                                     nFormat,
			                                                     oData.GetBuffer());
...
void CNetDDECltApp::ServerConnect(CNetDDEService* pService)
{
	...
	NetDDEPacketPtr requestPacket = EncodeClientConnectPacket(pService->m_oCfg.m_strRemName);
